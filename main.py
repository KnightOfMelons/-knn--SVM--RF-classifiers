...
# Вобще, если выбирать лучший классификатор, то у меня самый точный - это knn оказался. Остальные чуть хуже

# knn
# [Обучающая] Точность: 1.0000, Precision: 1.0000, Recall: 1.0000, F1-мера: 1.0000
# [Тестовая] Точность: 0.8824, Precision: 0.8852, Recall: 0.8824, F1-мера: 0.8830

# SVM
# [Обучающая] Точность: 0.9300, Precision: 0.9300, Recall: 0.9300, F1-мера: 0.9295
# [Тестовая] Точность: 0.8824, Precision: 0.8852, Recall: 0.8824, F1-мера: 0.8830
...
# Функции для оценки модели. ТУТ НАХОДЯТСЯ ТЕСТОВАЯ И ОБУЧАЮЩАЯ ВЫБОРКА
# Обучающая выборка используется для обучения модели, а тестовая выборка — для оценки её производительности на новых,
# невидимых данных. Это позволяет понять, насколько хорошо модель будет работать в реальных условиях.
...
while True:
    ...
    # Принцип работы: KNN — это метод, основанный на расстоянии. Он классифицирует новый объект на основе классов его
    # k ближайших соседей в пространстве признаков.
    if choose == 1:
        parameters = {
    ...
    # SVM ищет гиперплоскость, которая максимально разделяет классы в пространстве признаков. Использует концепцию
    # опорных векторов — точек, определяющих границу между классами.
    elif choose == 2:
        parameters = {
    ...
    # RF — это ансамблевый метод, который строит множество деревьев решений и объединяет их результаты для улучшения
    # точности и уменьшения переобучения.
    elif choose == 3:
        parameters = {
